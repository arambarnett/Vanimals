...
class SurfaceARView extends StatefulWidget {
  ...
  @override
  State<SurfaceARView> createState() => _SurfaceARViewState();
}

class _SurfaceARViewState extends State<SurfaceARView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          // AR View - This shows the live camera feed with plane detection
          ARView(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Place ${widget.vanimalName} on Surface',
                style: AppTheme.headlineSmall.copyWith(color: Colors.white),
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 10),
              Text(
                '1. Point camera at a flat surface (table, floor)\n'
                '2. Move device slowly to scan for planes\n'
                '3. Look for grid overlay on detected surfaces\n'
                '4. Tap on the grid to place your Vanimal\n'
                '5. Watch it anchor to the real surface!',
              ...
              ElevatedButton(
                ...
                child: const Text('Start Surface Detection'),
              ),
            ],
          ),
        ),
  ...
  Widget _buildSurfaceDetectionStatus() {
    return Positioned(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                planesDetected 
                  ? 'Surfaces detected! Tap on grid to place ${widget.vanimalName}'
                  : 'Scanning for surfaces... Move device slowly over flat areas',
  ...
  Widget _buildPlacementStatus() {
    return Positioned(
      ...
      child: Container(
        ...
        child: Text(
          '🎉 ${placedNodes.length} ${widget.vanimalName}${placedNodes.length > 1 ? 's' : ''} anchored',
          style: AppTheme.bodySmall.copyWith(
            color: Colors.white,
            fontWeight: FontWeight.bold,
          ),
  ...
  void _onARViewCreated(
    ...
    this.arSessionManager!.onInitialize(
      showFeaturePoints: false, // Hide feature points for cleaner view
      showPlanes: true, // Show detected planes as grid overlays
    ...
    this.arAnchorManager!.onPlaneDetected = _onPlaneDetected;
    
    setState(() {
      isLoading = false;
      statusText = "AR Ready - Point at flat surfaces";
    });
  }

  void _onPlaneDetected(ARPlaneAnchor anchor) {
    if (!planesDetected) {
      setState(() {
        planesDetected = true;
      });
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('🎯 Surface detected! Tap to place your Vanimal'),
  ...
  Future<void> _add3DVanimalNode(ARPlaneAnchor anchor) async {
    ...
    var vanimalNode = ARNode(
      ...
      uri: modelPath,
      scale: Vector3(0.15, 0.15, 0.15), // Good size for AR
